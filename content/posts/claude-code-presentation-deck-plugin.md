...
So I turned the prompt into a Claude Code **skill**, then packaged it as a **plugin**. Most of what I learned had little to do with deck styling. It was about where you draw the line between what the model decides and what a plain script does.
...
Decks are where confident fabrications survive review, because nobody fact-checks a bullet point on a slide.
...
Three string replacements, standard library only. Because the assembler is boring, none of its failure modes are interesting. It can't forget the print stylesheet or make up a font stack, because it isn't deciding those things.
...
Every deck is a single HTML file with Lato embedded as base64 woff2. That's around 725 KB, which is a lot for five slides.
